...
function VideoPlayer({ videoRef, stream, isMuted = false, isLocal = false, label, }) {
    const internalVideoRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        const videoElement = typeof videoRef === "function"
    ...
    return (<div className="relative w-full h-full bg-gray-800">
			<video ref={handleVideoRef} autoPlay playsInline muted={isMuted} className={`w-full h-full object-cover ${isLocal ? "scale-x-[-1]" : ""}`} style={{ minHeight: "300px", backgroundColor: "#1f2937" }}/>
